...
export const TV_ACCESS_ROLES: UserRole[] = [
  "SUPERADMIN",
  "ADMIN",
  "SUPPORT",
  "TV",
...
export const getSessionUser = cache(async () => {
  const sessionId = (await cookies()).get("vavive_session")?.value;
  if (!sessionId) return null;

  const session = await measureServerOperation("auth.session", () =>
...
export async function requireAnyRole(allowedRoles: readonly UserRole[]) {
  ...
  if (!hasAnyRole(user, allowedRoles))
    redirect(user.role === "TV" ? "/tv" : "/dashboard");
  return user;
}
...
export async function createSession(userId: string, isTV = false) {
  ...
  const duration = isTV ? 180 * 24 * 60 * 60 * 1000 : 8 * 60 * 60 * 1000;
  await prisma.session.create({
    data: { id, userId, expiresAt: new Date(Date.now() + duration) },
  });
  return id;
}
