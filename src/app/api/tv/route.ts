...
export async function GET() {
  ...
  const [monthContacts, latestContacts, previousQualified] = await Promise.all([
    ...
    prisma.contact.count({
      where: {
        contactedAt: { gte: previousMonthStart, lt: currentMonthStart },
        type: { in: QUALIFIED_CONTACT_TYPES },
      },
    }),
  ]);
  const monthlyByFranchisee = new Map<string, typeof monthContacts>();
  for (const contact of monthContacts)
    monthlyByFranchisee.set(contact.franchiseeId, [
      ...(monthlyByFranchisee.get(contact.franchiseeId) ?? []),
      contact,
    ]);
  const lastContactByFranchisee = new Map(
    latestContacts.map((item) => [item.franchiseeId, item._max.contactedAt]),
  );
  const qualified = monthContacts.filter((contact) =>
    QUALIFIED_CONTACT_TYPES.includes(contact.type),
  );
  const countType = (type: string) =>
    monthContacts.filter((contact) => contact.type === type).length;
  return NextResponse.json({
      ...
      return {
        ...
        attention:
          daysWithoutContact === null
            ? "red"
            : daysWithoutContact <= CONTACT_ATTENTION.warningAfterDays
              ? "green"
              : daysWithoutContact <= CONTACT_ATTENTION.dangerAfterDays
                ? "yellow"
                : "red",
      };
    }),
