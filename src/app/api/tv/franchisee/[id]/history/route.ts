...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const contacts = await prisma.contact.findMany({ where: { franchiseeId: id }, orderBy: [{ contactedAt: "desc" }, { createdAt: "desc" }], take: 5, select: { id: true, type: true, contactedAt: true, user: { select: { name: true } }, member: { select: { name: true } } } });
