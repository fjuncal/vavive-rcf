...
import { LayoutDashboard, LayoutGrid, MonitorPlay, Rows3 } from "lucide-react";
import { useEffect, useState } from "react";
export default function TVModePage() {
  const [role, setRole] = useState<string | null>(null);
  useEffect(() => { fetch("/api/auth/me").then((response) => response.ok ? response.json() : null).then((data) => setRole(data?.user?.role ?? null)).catch(() => setRole(null)); }, []);
  return (
    <main className="min-h-screen bg-[#eef7ef] p-6 text-[#003b71] lg:p-10">
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-6xl flex-col justify-center">
        {role && role !== "TV" && <div className="mb-6 flex justify-end"><Link href="/dashboard" className="inline-flex items-center gap-2 rounded-xl bg-[#003b71] px-4 py-3 text-sm font-bold text-white"><LayoutDashboard className="h-4 w-4"/>Ir para o painel</Link></div>}
